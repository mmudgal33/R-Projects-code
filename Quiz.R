...
ggplot(h_train,aes(x=Distance))+geom_density(color="red")+
  stat_function(fun=dnorm,args=list(mean=mean(h_train$Distance),sd=sd(h_train$Distance)),color="green")
 


library(ggplot2)
ggplot(h_train,aes(x=Price,y=Distance))+geom_point()
...
#  select(-etc)
table(h_train$Type)
n(unique(h_train$Postcode))


sell=tapply(h_train$Price,h_train$SellerG,sum,na.rm=T)
sell1=as.data.frame(sell)
which.max(sell1$sell)

council=tapply(h_train$Price,h_train$CouncilArea,var,na.rm=T)
council1=as.data.frame(council)
which.max(council1$council)

#h_all[h_all$Method=="S",]  #6032 rows have s so remove

unique(h_train$Address)

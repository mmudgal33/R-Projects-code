...
h_train_na=read.csv("housing_train.csv",stringsAsFactors = F)
...
lapply(h_all, function(x) length(is.na(x)))
...
for(col in column){
  ...
  if(sum(is.na(h_all[,col]))>0){
    ...
    h_all[is.na(h_all[,col]),col]=mode(h_all[,col])
  }
...
#remove address
sum(is.na(h_all$Address))
table(h_all$Address)
h_all$Address=NULL

#removing na from predictors data numeric
for(col in c("Landsize","BuildingArea","YearBuilt")){
...
#removing na from predictors data
for(col in c("YearBuilt")){
  
  if(sum(is.na(h_all[,col]))>0){
    
    h_all[is.na(h_all[,col]),col]=round(mean(h_all[,"YearBuilt"],na.rm=T),0)
  }
...
fit=lm(price ~ Distance + Postcode + Landsize + BuildingArea + YearBuilt + 
         Type_h + Type_t + Rooms_2 + Rooms_4 + Rooms_5 + 
         Method_S + Method_SP + councilarea_6 + councilarea_9 + councilarea_10 + 
         councilarea_11 + councilarea_12 + 
         Bedroom2_1 + Bedroom2_3 + Bedroom2_4 + Bedroom2_OT + car_0 + 
         car_4 + Suburb_Armadale + Suburb_Williamstown + 
         ...
         Suburb_BalwynNorth + 
         Suburb_Northcote + Suburb_Kew + Suburb_Brighton + 
         Suburb_GlenIris + Suburb_Essendon + Suburb_Brunswick + 
...
fit.final=fit=lm(price ~ Distance + Postcode + Landsize + BuildingArea + 
...
write.csv(test.pred,"housing_project_111.csv",row.names = F)

m=read.csv("housing_project_111.csv")
sum(is.na(m[,1]))
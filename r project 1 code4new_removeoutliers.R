...
h_train1=h_train[h_train$Price<4000000,]
library(ggplot2)
...
fit=lm(Price ~ Type_h + Type_t + Rooms_2 + Rooms_4 + Rooms_5 + 
         Bedroom2_1 + Bedroom2_4 + Bedroom2_o + car_0 + car_4 + Suburb_Williamstown + 
         Suburb_Ivanhoe + Suburb_Toorak + Suburb_BrunswickWest + Suburb_KeilorEast + 
         Suburb_HawthornEast + Suburb_SurreyHills + 
         Suburb_Kensington + Suburb_Doncaster + 
         Suburb_Footscray + Suburb_MooneePonds + 
         Suburb_Balwyn + Suburb_Hampton + Suburb_Yarraville + 
         Suburb_Camberwell + 
         Suburb_BrightonEast + Suburb_Hawthorn + Suburb_BalwynNorth + 
         Suburb_Kew + Suburb_Brighton + 
         Suburb_Glenroy + Suburb_GlenIris + Suburb_Essendon + 
         Suburb_Brunswick + Suburb_Preston + Suburb_BentleighEast + 
         Suburb_Reservoir + SellerG_Village + SellerG_Kay + 
         SellerG_Miles + SellerG_Greg + SellerG_RT + 
         SellerG_Fletchers + SellerG_Marshall + 
         SellerG_Jellis + SellerG_Nelson + 
         CouncilArea_Manningham + CouncilArea_Brimbank + 
         CouncilArea_HobsonsBay + CouncilArea_Melbourne + CouncilArea_Banyule + 
...
fit.final=fit=lm(price ~ Distance + Postcode + Type_h + 
                   Type_t + Rooms_1 + Rooms_2 + Rooms_4 + Rooms_5 + Method_PI + 
                   Method_S + Method_SA + Method_SP + Bedroom2_3 + 
                   Bedroom2_4 + Bedroom2_OT + car_0 + car_4 + 
                   Suburb_Williamstown + Suburb_Melbourne + 
                   Suburb_SunshineWest + Suburb_Ivanhoe + Suburb_TemplestoweLower + 
                   Suburb_KeilorEast + 
                   Suburb_Prahran + Suburb_SurreyHills + Suburb_Kensington + 
                   Suburb_Sunshine + Suburb_Elwood + Suburb_Newport + 
                   Suburb_Doncaster + 
                   Suburb_Hampton + 
                   Suburb_Yarraville + Suburb_Balwyn + Suburb_MalvernEast + 
                   Suburb_Camberwell + Suburb_Carnegie + 
                   Suburb_Bentleigh + Suburb_BrightonEast + 
                   Suburb_Hawthorn + Suburb_BalwynNorth + 
                   Suburb_Brighton + Suburb_Glenroy + 
                   Suburb_Essendon + Suburb_Brunswick + Suburb_SouthYarra + 
                   Suburb_Preston + Suburb_Richmond + Suburb_BentleighEast + 
                   Suburb_Reservoir + SellerG_Village + 
                   SellerG_Kay + 
                   SellerG_Gary + SellerG_Miles + 
                   SellerG_Fletchers + 
                   SellerG_Buxton + 
                   SellerG_Marshall + 
                   SellerG_Jellis + SellerG_Nelson + CouncilArea_Monash + 
                   ...
                   CouncilArea_Darebin + CouncilArea_MooneeValley + 
                   CouncilArea_Moreland,data=h_train1)

fit.final=step(fit.final)

summary(fit.final)
#after fit formula used
fit.final=fit=lm(price ~ Distance + Postcode + Type_h + Type_t + Rooms_1 + Rooms_2 + 
                   Rooms_4 + Rooms_5 + Method_PI + Method_S + Method_SA + Method_SP + 
                   Bedroom2_3 + Bedroom2_4 + car_0 + car_4 + Suburb_Williamstown + 
                   Suburb_Melbourne + Suburb_SunshineWest + Suburb_Ivanhoe + 
                   Suburb_TemplestoweLower + Suburb_KeilorEast + Suburb_Prahran + 
                   Suburb_SurreyHills + Suburb_Kensington + Suburb_Sunshine + 
                   Suburb_Elwood + Suburb_Newport + Suburb_Doncaster + Suburb_Hampton + 
                   Suburb_Yarraville + Suburb_Balwyn + Suburb_MalvernEast + 
                   Suburb_Carnegie + Suburb_Bentleigh + Suburb_BrightonEast + 
                   Suburb_Hawthorn + Suburb_BalwynNorth + Suburb_Brighton + 
                   Suburb_Glenroy + Suburb_Essendon + Suburb_Brunswick + Suburb_SouthYarra + 
                   Suburb_Preston + Suburb_Richmond + Suburb_BentleighEast + 
                   Suburb_Reservoir + SellerG_Village + SellerG_Kay + SellerG_Gary + 
                   SellerG_Miles + SellerG_Fletchers + SellerG_Buxton + SellerG_Marshall + 
                   SellerG_Jellis + SellerG_Nelson + CouncilArea_Monash + CouncilArea_Manningham + 
                   ...
                   CouncilArea_Yarra + CouncilArea_Darebin + CouncilArea_MooneeValley + 
                   CouncilArea_Moreland, data = h_train1)
...
rf.model=randomForest(price~.,data=h_train1,do.trace=T)
test.price=predict(rf.model,newdata= h_test)
write.csv(test.price,"housing_project_removeoutliers_submit.csv",row.names = F)
